...
// Обрабатываем клик по элементу галереи
galleryContainer.addEventListener('click', onGalleryItemClick);

function onGalleryItemClick(event) {
  // Запрещаем стандартное поведение (загрузку изображения)
  ...
  const instance = basicLightbox.create(`
    <div class="modal">
      ...
      <p class="modal-caption">${imageDescription}</p>
    </div>
  `, {
    onShow: (instance) => {
    ...
    onClose: (instance) => {
      // Удаляем обработчик клавиши Escape при закрытии модального окна
      document.removeEventListener('keydown', onEscKeyPress);
    }
  });
  
  // Функция для закрытия модального окна по клавише Escape
